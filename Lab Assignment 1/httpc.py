...
def get(url,v):
   ...
   # 3: Use GET to request from the server and \r\n\r\n to send the request
   req = "GET " + url.path + "?" + url.query + " HTTP/1.1\r\nHost: " + url.hostname + "\r\n\r\n"
   # 4: Send out the request
   listener.sendall(req.encode("utf-8"))
   #5: Response variable receives the response of max length 4096
...
def post(url, header, data, v):
   ...
   # 2: Connect to the server at port 80
   listener.connect((url.hostname, 80))
   # 3: Use POST to request send information to the server and \r\n\r\n to send the request
